...
This module integrates Sigma rules for parsing and evaluating logs.
...
type AutoGenerated struct {
    ...
    Detection struct {
        Selection struct {
            CsMethod string `yaml:"cs-method"`
        } `yaml:"selection"`
        Keywords         []string `yaml:"keywords"`
        FilterMainStatus struct {
            ScStatus int `yaml:"sc-status"`
        } `yaml:"filter_main_status"`
