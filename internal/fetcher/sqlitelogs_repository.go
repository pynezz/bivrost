...
func (r *SQLiteLogsRepository) Create(log NginxLog) (*NginxLog, error) {
	res, err := r.db.Exec(
		`INSERT INTO nginx_logs (
			time_local, remote_addr, remote_user, request, status, body_bytes_sent, request_time, http_referer, http_user_agent, request_body
		) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
func (r *SQLiteLogsRepository) GetByIP(ip string) (NginxLogsList, error) {
	var logs NginxLogsList // Just a slice of logs ([]NginxLog)
	rows, err := r.db.Query(`SELECT * FROM nginx_logs WHERE remote_addr = ?`, ip)
	if err != nil {
